...
class TransformerBlock1(nn.Module):
    ...
    def __init__(
        ...
        self.epa_block = EGCA(input_size=input_size, hidden_size=hidden_size, proj_size=proj_size, num_heads=num_heads,
                              channel_attn_drop=dropout_rate, spatial_attn_drop=dropout_rate)

        self.conv3 = nn.Sequential(nn.Dropout2d(0.1, False), nn.Conv2d(hidden_size, hidden_size, 3, padding=1))
    ...
    def forward(self, x):
        ...
        attn = x + self.epa_block(self.norm(x))

        attn_skip = attn.reshape(B, H, W, C).permute(0, 3, 1, 2)  # (B, C, H, W, D)
        attn = self.conv3(attn_skip)
